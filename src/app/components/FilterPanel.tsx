    ...
    <div className="fixed inset-0 bg-black/50 z-[9999] flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="bg-white w-full md:max-w-md md:rounded-lg max-h-[85vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <Card>
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  <Label htmlFor="available" className="cursor-pointer">Sadece Müsait Olanlar</Label>
                  <p className="text-xs text-muted-foreground mt-1">En az bir şarj noktası müsait olan istasyonları göster</p>
                </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {availableBrands.map((brand) => (
                <Badge key={brand} variant={filters.brands.includes(brand) ? 'default' : 'outline'} className="cursor-pointer" onClick={() => toggleBrand(brand)}>
                  {brand}
                </Badge>
              ))}
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">{filters.minPower > 0 ? `${filters.minPower} kW` : 'Tümü'}</span>
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2">
              {connectorTypes.map((connector) => (
                <Button key={connector} variant={filters.connectorTypes.includes(connector) ? 'default' : 'outline'} size="sm" onClick={() => toggleConnector(connector)} className="justify-start">
                  {connector}
                </Button>
              ))}
            </div>
        ...
        <div className="p-4 border-t bg-gray-50 flex gap-3">
          <Button variant="outline" className="flex-1" onClick={resetFilters}>Sıfırla</Button>
          <Button className="flex-1" onClick={onClose}>Uygula</Button>
        </div>
      </div>
    </div>
  );
